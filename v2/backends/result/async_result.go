...
import (
	...
	resultv1 "github.com/RichardKnop/machinery/v1/backends/result"
...
func (asyncResult *AsyncResult) Touch() ([]reflect.Value, error) {
	if asyncResult.backend == nil {
		return nil, resultv1.ErrBackendNotConfigured
	}
	...
	if asyncResult.taskState.IsSuccess() {
		vals := make([]reflect.Value, 0, len(asyncResult.taskState.Results))
		for _, r := range asyncResult.taskState.Results {
...
func (asyncResult *AsyncResult) GetWithTimeout(timeoutDuration, sleepDuration time.Duration) ([]reflect.Value, error) {
	...
	for {
		select {
		case <-timeout.C:
			return nil, resultv1.ErrTimeoutReached
		default:
			results, err := asyncResult.Touch()
